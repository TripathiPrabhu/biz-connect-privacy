...
export const sendDeletionRequest = async (req, res) => {
    try {
        const { fullName, email, mobileNumber, gender } = req.body;

        if (!fullName || !email || !mobileNumber || !gender) {
            return res.status(400).json({ error: 'All fields are required: fullName, email, mobileNumber, and gender.' });
        }

        // Call the EmailAuth service to send the deletion request email
        await emailAuth.sendDeletionRequest({ fullName, email, mobileNumber, gender });

        res.json({ message: 'Data deletion request sent successfully.' });
    } catch (error) {
        console.error('Error sending data deletion request:', error);
        res.status(500).json({ error: 'Failed to send data deletion request.' });
    }
};
